...
DEFAULT_ENABLE_TYPE1_LOGIC = os.environ.get("PHYSICS_ENABLE_TYPE1_LOGIC", "true").lower() in {
    "1",
    "true",
    "yes",
    "y",
}
DEFAULT_TYPE1_MODEL = os.environ.get("PHYSICS_TYPE1_MODEL", "Qwen/Qwen2.5-Coder-7B-Instruct")
DEFAULT_TYPE1_BASE_URL = os.environ.get("PHYSICS_TYPE1_BASE_URL", DEFAULT_POLISH_BASE_URL).rstrip("/")
DEFAULT_TYPE1_PROXY_URL = os.environ.get("PHYSICS_TYPE1_PROXY_URL", "").strip()
DEFAULT_TYPE1_MAX_TOKENS = int(os.environ.get("PHYSICS_TYPE1_MAX_TOKENS", "1024"))
DEFAULT_TYPE1_TIMEOUT_SECONDS = float(os.environ.get("PHYSICS_TYPE1_TIMEOUT_SECONDS", "30"))
DEFAULT_TYPE1_Z3_TIMEOUT_MS = int(os.environ.get("PHYSICS_TYPE1_Z3_TIMEOUT_MS", "5000"))
...
try:
    from physics_type1_logic import run_type1_logic as run_type1_logic_reasoner
except Exception:
    run_type1_logic_reasoner = None
...
def _call_type1_chat_completion(
    messages: list[Dict[str, str]],
    model_name: str,
    max_tokens: Optional[int] = None,
    timeout_seconds: Optional[float] = None,
    ...
    payload = {
        ...
        "messages": messages,
        "temperature": 0,
        "max_tokens": int(max_tokens or DEFAULT_TYPE1_MAX_TOKENS),
    }
    data = json.dumps(payload).encode("utf-8")
    req = urllib.request.Request(
        f"{DEFAULT_TYPE1_BASE_URL}/chat/completions",
        data=data,
        headers={"Content-Type": "application/json"},
        method="POST",
    )

    with urllib.request.urlopen(req, timeout=float(timeout_seconds or DEFAULT_TYPE1_TIMEOUT_SECONDS)) as resp:
        body = resp.read().decode("utf-8")
    parsed = json.loads(body)
    return str(parsed["choices"][0]["message"]["content"]).strip()
...
def _try_direct_text_guardrail(question: str) -> Optional[Dict[str, Any]]:
    ...
    flux_values = _direct_question_values(question, {"Wb", "mWb", "uWb", "nWb"})
    time_values = _direct_question_values(question, {"s", "ms", "us"})

    # Faraday coil with flux change per turn. This catches natural wording such
    # as "500 turns; flux through each turn decreases from 8 mWb to 2 mWb
    # during 0.03 s", where the extraction adapter may output an invalid target.
    if (
        len(flux_values) >= 2
        and time_values
        and ("induced emf" in text or "electromotive force" in text or "average emf" in text)
        and "flux" in text
        and ("turn" in text or "coil" in text)
    ):
        turns = 1.0
        turn_match = re.search(r"([+-]?\d+(?:\.\d+)?)\s*(?:turns?|windings?)\b", question, flags=re.I)
        if turn_match:
            try:
                turns = float(turn_match.group(1))
            except Exception:
                turns = 1.0
        delta_phi = abs(flux_values[-1] - flux_values[0])
        value = turns * delta_phi / time_values[0]
        return _direct_formula_result(
            _format_number(value),
            "V",
            "direct_guardrail_faraday_coil_flux_change",
            "induction",
            f"Use Faraday's law for a coil: |e| = N|Delta Phi|/Delta t = {turns:.6g}*{delta_phi:.6g}/{time_values[0]:.6g} = {value:.6g} V",
            confidence=0.94,
        )
...
def _apply_optional_polish(
    ...
    if not _string_answer(out.get("answer")):
        out["polish_status"] = "skipped_no_answer"
        out["polish_error"] = None
        return out
...
def _has_type1_logic_payload(data: Dict[str, Any]) -> bool:
    premises = data.get("premises") or data.get("premises-NL") or data.get("premises_nl")
    return isinstance(premises, list) and len(premises) > 0


def _type1_proxy_predict_url() -> str:
    url = DEFAULT_TYPE1_PROXY_URL.strip()
    if not url:
        return ""
...
def _try_type1_proxy(data: Dict[str, Any], debug: bool = False) -> tuple[Optional[list[Dict[str, Any]]], str]:
    url = _type1_proxy_predict_url()
    if not url:
        return None, "type1_proxy_not_configured"
    try:
        payload = dict(data)
        payload.setdefault("debug", debug)
        req = urllib.request.Request(
            url,
            data=json.dumps(payload).encode("utf-8"),
            headers={"Content-Type": "application/json"},
        ...
        with urllib.request.urlopen(req, timeout=DEFAULT_TYPE1_TIMEOUT_SECONDS) as resp:
            body = resp.read().decode("utf-8")
        parsed = json.loads(body)
    except Exception as exc:
        return None, f"type1_proxy_failed:{exc}"

    if isinstance(parsed, dict):
        parsed = [parsed]
    if not isinstance(parsed, list) or not parsed:
        return None, "type1_proxy_bad_response"

    if isinstance(parsed[0], dict):
        _normalize_type1_proxy_answer(parsed[0], data)

    if debug and isinstance(parsed[0], dict):
        parsed[0].setdefault("debug", {})
        if isinstance(parsed[0]["debug"], dict):
            parsed[0]["debug"].setdefault("method", "type1_7b_proxy")
            parsed[0]["debug"]["type1_proxy_url_configured"] = True
    return parsed, "ok"


def _normalize_type1_proxy_answer(result: Dict[str, Any], request_data: Dict[str, Any]) -> None:
    ...
    question = str(request_data.get("query") or request_data.get("question") or "")
    ...
    for item in options:
        if isinstance(item, dict):
            text = str(item.get("text") or item.get("option") or item.get("value") or item.get("answer") or "").strip()
        else:
            text = str(item).strip()
        text = re.sub(r"^[A-Da-d][\.\)]\s*", "", text).strip()
        option_texts.append(text)

    normalized = [text.lower() for text in option_texts]
    yes_no_set = {"yes", "no", "unknown", "uncertain"}
    if not option_texts or not set(normalized).issubset(yes_no_set):
        return
    if re.search(r"(^|\n)\s*[A-Da-d][\.\)]\s+", question):
        return
    ...
    if 0 <= idx < len(option_texts):
        mapped = option_texts[idx]
        result["answer"] = "Unknown" if mapped.lower() == "uncertain" else mapped


def _try_type1_logic_reasoner(data: Dict[str, Any], debug: bool = False) -> tuple[Optional[list[Dict[str, Any]]], str]:
    if not DEFAULT_ENABLE_TYPE1_LOGIC:
        return None, "type1_logic_disabled"
    if run_type1_logic_reasoner is None:
        return None, "type1_logic_module_unavailable"
    if not _has_type1_logic_payload(data):
        return None, "type1_logic_no_premises"
    try:
        result = run_type1_logic_reasoner(
            payload=data,
            chat_fn=_call_type1_chat_completion,
            model_name=DEFAULT_TYPE1_MODEL,
            max_tokens=DEFAULT_TYPE1_MAX_TOKENS,
            timeout_seconds=DEFAULT_TYPE1_TIMEOUT_SECONDS,
            z3_timeout_ms=DEFAULT_TYPE1_Z3_TIMEOUT_MS,
            debug=debug,
        )
    except Exception as exc:
        return None, f"type1_logic_failed:{exc}"
    if not isinstance(result, list) or not result:
        return None, "type1_logic_empty_result"
    return result, "ok"
...
def _handle_competition_predict(data: Dict[str, Any]):
    ...
    if query_type == "type1":
        type1_proxy_out, type1_proxy_status = _try_type1_proxy(data, debug=debug)
        if type1_proxy_out is not None:
            return jsonify(_json_safe(type1_proxy_out))

        type1_logic_out, type1_logic_status = _try_type1_logic_reasoner(data, debug=debug)
        if type1_logic_out is not None:
            return jsonify(_json_safe(type1_logic_out))
        ...
        if chlt_out is not None:
            if debug:
                ...
                chlt_out["type1_logic_status"] = type1_logic_status
                chlt_out["type1_proxy_status"] = type1_proxy_status
        ...
        if debug:
            ...
            fallback[0]["debug"]["type1_logic_status"] = type1_logic_status
            fallback[0]["debug"]["type1_proxy_status"] = type1_proxy_status
    ...
    return jsonify(
        {
            ...
            "type1_logic_enabled": DEFAULT_ENABLE_TYPE1_LOGIC,
            "type1_logic_model": DEFAULT_TYPE1_MODEL,
            "type1_logic_base_url": DEFAULT_TYPE1_BASE_URL,
            "type1_logic_module_ready": run_type1_logic_reasoner is not None,
            "type1_proxy_enabled": bool(_type1_proxy_predict_url()),
            "type1_logic_timeout_seconds": DEFAULT_TYPE1_TIMEOUT_SECONDS,
            "llm_fallback_enabled": DEFAULT_USE_LLM_FALLBACK,
